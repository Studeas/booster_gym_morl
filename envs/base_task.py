...
class BaseTask:
    ...
    def create_sim(self):
        ...
        if sim_cfg["physics_engine"] == "physx":
            ...
            raise ValueError(
                f"Invalid physics engine backend: {sim_cfg['physics_engine']}"
            )
        ...
        if self.cfg["viewer"]["record_video"]:
            ...
            if self.camera is None:
                ...
                self.camera = self.gym.create_camera_sensor(
                    self.envs[self.cfg["viewer"]["record_env_idx"]], camera_props
                )
                self.camera_frames = []
            cam_pos = gymapi.Vec3(
                *(
                    ...
                    for x, y in zip(
                        self.root_states[
                            self.cfg["viewer"]["record_env_idx"], 0:3
                        ].tolist(),
                        self.cfg["viewer"]["pos"],
                    )
                )
            ...
            cam_target = gymapi.Vec3(
                *self.root_states[self.cfg["viewer"]["record_env_idx"], 0:3].tolist()
            )
            self.gym.set_camera_location(
                self.camera,
                self.envs[self.cfg["viewer"]["record_env_idx"]],
                cam_pos,
                cam_target,
            )
